...
description: "Pull Docker images from private registries including Docker Hub and Amazon ECR"
---
...
time="2025-01-29T21:22:15Z" level=fatal msg="Error: image yourname/postgres:latest not found"
ERROR: exit status 1
```
...
> **Note:** You do not need to include the `https://` protocol as part of the registry address. Convox will add this for you automatically.
...
> **Note:** Local racks do not authenticate against Amazon EC2 Container Registries (ECR) but do respect `docker login` sessions. You can authenticate against your chosen ECR's with `docker login`/`aws ecr get-login-password` (see [here](https://aws.amazon.com/blogs/compute/authenticating-amazon-ecr-repositories-for-docker-cli-with-credential-helper/)) before running a local build and your remote images will be pulled successfully.
