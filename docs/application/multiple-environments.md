...
Because `convox.yml` is version controlled and environment changes are recorded through [Releases](/deployment/releases), you can replicate any specific environment setup at will, which is useful for disaster recovery or debugging.
...
Variables listed in the top-level `environment` section (`TIMER_SCHEDULE`, `WEB_COMMAND`) are required, so the App will not start unless they are set.
